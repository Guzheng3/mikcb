...
| 项目 | 规范 |
|---|---|
| 圆角 | 卡片外层 24dp，课程块 16dp；两列之间保留 10-12dp 间隔，避免出现内嵌小圆角。 |
| 边缘 | 使用 1dp 半透明描边加极轻外阴影；亮色用白色 40% 描边，暗色用黑色 32% 描边，不加重边框。 |
| 背景 | 复用 qingyu 桌面卡片的磨砂底；上方保留一层 4% 的顶部高光，让边缘不生硬。 |
| 留白 | 外边距 14dp，列头到课程区 8dp，课程块上下 6dp；文字不贴边。 |
| 字体 | 用户名 13sp 中黑，课名 14sp 中黑，时间/教室 11sp 常规；优先用系统无衬线字体。 |
| 色彩 | 双方各用一个柔和主题色；只给用户名圆点和课程块左侧色条，不做整块大面积撞色。 |
| 明暗模式 | 亮暗两套背景、描边、文字色；主题色跟随桌面卡片现有深色映射，保证对比度足够。 |
| 显示保护 | 课名最多 2 行，教室和时间最多 1 行，超出省略；固定课程块高度，避免刷新时跳动。 |
| 空状态 | 无课显示一个居中的浅色「今天没课」；对方未同步则显示用户名和「待同步」，不空白。 |

### 6.4 显示效果要求

1. 卡片首次加载使用最后一次同步快照，不显示空白。
2. 刷新时只更新数据文本，不重建整套布局，避免闪烁。
3. 用户名列、课程区域、整卡轮廓都保持可点击热区，最小 44dp 高。
4. 长用户名缩写或省略，不挤占课程内容。
5. 同一时刻只有一个课程块高亮，下一节课用左侧色条加轻微底色突出。
