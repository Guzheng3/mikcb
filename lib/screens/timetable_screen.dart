...
class _TimetableScreenState extends State<TimetableScreen>
  ...
  Widget _buildHomeTitle(
    ...
    if (provider.settings.coupleTimetableOverlayEnabled) {
      ...
      // 本机仍保留已导入的对方课表（provider.hasPartnerBinding），但它不再
      // 撑起情侣标题——否则用户会以为「退出登录没生效」。TA 的课表可以从
      // 课表管理页（profiles）继续进入。
  ...
  Future<void> _openWithuCoupleLogin() async {
    ...
    // 凭证刚由 connect 落盘，直接按本地凭证收敛登录态，不再多发一次 bootstrap。
    ...
    return Stack(
      ...
      children: [
        Positioned(
          ...
          child: AnimatedBuilder(
            // 动画期间时间轴由 deck 卡内的副本负责（随卡片一起缩放/位移）。
            // 固定轨在 deck 激活或停靠挂起时隐藏，静止时恢复。
  ...
  /// 时间轴在切周动画期间由卡片内副本负责（跟随卡片一起缩放/位移/淡入）。
  /// 静止时固定轨恢复。此方法计算卡片内时间轴的位置与节高，与固定轨
  /// （_buildWeekPager 顶部的 Positioned）保持同一坐标基准。
  ...
  /// 单张切周卡片：课表区 + 卡片内时间轴副本，二者作为整体参与缩放/
  /// 位移/淡入（时间轴随课表一起滑动和缩放）。
  ...
  void _handleDockTap(String id, TimetableSettings settings) {
    ...
    switch (id) {
      ...
      default:
        ...
        // 的走普通推入。再点当前内嵌页与日/周 Tab 同口径无动作，其余
  ...
  Future<void> _showProfileQuickSwitchSheet() async {
    ...
    final selected = await showProfileQuickSwitchSheet(
      context,
      // 情侣课表开关开启且 TA 课表同为可切换课表（情侣标题的卡片右半
  ...
  /// 情侣标题长按：只给「历史课表 / 课表管理」两个直达入口，
  /// 不再弹出「切换课表」的 profile 列表。课表管理以弹窗打开。
  ...
  /// 课表管理：以弹窗形式打开（满屏 Dialog），而不是推进管理子页导航栈。
