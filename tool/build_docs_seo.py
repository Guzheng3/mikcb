...
TOPIC_PAGES = (
    {
        ...
        "description": "轻屿课表是一款免费开源的 Android 课表 App，支持教务导入、ICS、AI 识图、课程提醒、桌面小组件和本地数据管理。",
        ...
        "sections": (
            ...
            ("适合哪些学生使用？", "如果你希望减少手动录入、快速知道下一节课、在桌面直接查看课程，或需要在手机与电脑之间编辑课表，轻屿课表提供了从教务系统导入到日常提醒的一整套工具。没有学校适配时，也可以使用 ICS、AI 识图或表格模板导入。", ("新学期快速建立课表", "需要课程提醒和桌面小组件", "重视本地数据与备份", "需要跨设备同步")),
        ),
    },
    {
        ...
        "sections": (
            ("课前、课中、下课连续显示", "超级岛不是一条孤立通知，而是围绕一节课持续变化的状态。它可以展示课程名称、上课地点、开始时间和剩余进度，并在课程结束后切换到下一节安排。", ("课前倒计时与课程地点", "课中状态和进度提示", "下课后承接下一节课", "状态随课程阶段更新")),
    ...
    },
    {
        ...
        "sections": (
            ...
            ("备份与数据安全建议", "课表数据可以通过 WebDAV 同步和备份。重要修改前请先手动备份，并检查同步内容是否符合预期。WebDAV 地址、账号和密码由用户自行配置，请使用可信服务并定期检查访问权限。", ("重要修改前先手动备份", "不要公开分享 WebDAV 凭据", "优先使用 HTTPS 服务地址", "定期保留离线备份")),
        ),
    },
)
